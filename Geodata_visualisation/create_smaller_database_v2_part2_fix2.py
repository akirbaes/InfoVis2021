...
names_database = dict()
for pair in database_names:
    names_database[database_names[pair]]=pair

path="SMALLER_DATABASE_CLEAN"
outpath="SMALLER_DATABASE_CLEAN_2"
...
for dirpath, dirnames, filenames in os.walk(path):
    for name in filenames:
        if (name.endswith(".shp")):
            ...
            shortname = name[:-4]
            column,value = names_database[shortname]
            db = db[db[column]==value]
            print("Selected only",shortname,value,"=",len(db))
